...
async function dismissDiagnosticIfPresent(page) {
  const btn = page.locator(SELECTORS.diagnosticCancel);
...
async function scrapeTest(page, testName) {
  ...
  while (true) {
    ...
    // G1 shows a "Diagnostic Warning" interstitial after the last question — dismiss it
    await dismissDiagnosticIfPresent(page);
...
async function main() {
  ...
  // Navigate to homepage — Car Tests is the default tab, no tab click needed
  await page.goto(HOME_URL, { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(800);

  // Click the "Next: G1 Car License Practice Test 1" button
  ...
  for (let testIndex = 1; testIndex <= TOTAL_TESTS; testIndex++) {
    const testName = `g1-practice-test-${testIndex}`;
    console.log(`\n--- Starting ${testName} ---`);
  ...
  fs.writeFileSync(
    path.join(OUTPUT_DIR, "g1-all-questions.json"),
